...
async function scheduleReel(page, post, { dryRun, shot }) {
  ...
  // Close any @mention typeahead before clicking on: accepting its first
  // suggestion rewrites the handle into a mention of another account.
  for (let i = 0; i < 3; i++) {
  ...
  const reelCaption = await page.evaluate(() =>
    (document.querySelector('[contenteditable="true"]') || {}).innerText || '');
  const lostHandles = (post.caption.match(/@[A-Za-z0-9_.]+[A-Za-z0-9_]/g) || [])
    .filter(h => !reelCaption.includes(h));
  if (lostHandles.length) {
    await shot(page, 'FAIL_reel_caption_changed');
    throw new Error(`Caption changed in the composer (missing: ${lostHandles.join(', ')})`);
  }
  ...
  // A "You scheduled a post" promo modal can sit over the composer: that is success.
  // Reels can sit on "Scheduling options" for well over two minutes while the
  // upload finalises, then land on the Scheduled list. Wait long, and count that
  // landing as success.
  const closed = await page.waitForFunction(() => {
    const body = document.body.innerText || '';
    return /You scheduled a post/i.test(body)
      || /scheduled_posts|content_calendar/.test(location.href)
      || !/Scheduling options/i.test(body);
  }, { timeout: 360000, polling: 3000 }).then(() => true).catch(() => false);
  ...
  await sleep(2000);
  if (!closed) throw new Error('Reel composer still open after 6 min - UNCONFIRMED. Check Business Suite > Scheduled before retrying, or it may double-post.');
