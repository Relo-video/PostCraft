...
/**
 * Close any open popup (suggestion list, picker) by clicking a plain section
 * heading. NEVER press Escape in TikTok Studio: it opens "Sure you want to
 * cancel your upload?".
 */
async function clickNeutral(page) {
  const pt = await page.evaluate(() => {
    const h = Array.from(document.querySelectorAll('div,span,h1,h2,h3,p'))
      .find(e => /^(Settings|Details)$/.test((e.innerText || '').trim()) && !e.children.length
                 && e.getBoundingClientRect().width > 0 && e.getBoundingClientRect().top > 60);
    if (!h) return null;
    const r = h.getBoundingClientRect();
    return { x: r.left + 5, y: r.top + r.height / 2 };
  });
  if (pt) { await page.mouse.click(pt.x, pt.y); await sleep(800); }
}
...
async function setDate(page, post, shot) {
  ...
  // Read the month from the calendar popup only, never from the whole page.
  const calMonth = () => page.evaluate(() => {
    const cal = Array.from(document.querySelectorAll('div')).find(e => {
      const r = e.getBoundingClientRect();
      return /[A-Z][a-z]+\s*\/\s*\d{4}/.test(e.innerText || '') && /Sun/.test(e.innerText || '')
        && r.width > 200 && r.width < 700;
    });
    if (!cal) return null;
    const m = cal.innerText.match(/([A-Z][a-z]+)\s*\/\s*(\d{4})/);
    return `${m[1]} / ${m[2]}`;
  });

  // Open the popup; a click can miss while another picker is closing, so retry.
  let shown = null;
  for (let a = 0; a < 3 && !shown; a++) {
    if (a > 0) await clickNeutral(page);
    f = await readFields(page);
    if (!f.date) break;
    await page.mouse.click(f.date.x, f.date.y);
    await sleep(3000);
    shown = await calMonth();
  }
  if (!shown) { await shot(page, 'FAIL_no_calendar'); throw new Error('Calendar popup did not open'); }

  // Month navigation clicks ONLY the rightmost small control inside the popup.
  // (The old version clicked the last small element on the whole page, which
  // once hit the upload's cancel control.)
  for (let i = 0; shown !== header; i++) {
    if (i === 12) { await shot(page, 'FAIL_month'); throw new Error(`Could not reach ${header} (showing ${shown})`); }
    const clicked = await page.evaluate(() => {
      ...
      if (!cal) return false;
      const top = cal.getBoundingClientRect().top;
      const nav = Array.from(cal.querySelectorAll('svg,span,button'))
        .filter(e => { const r = e.getBoundingClientRect(); return r.width > 0 && r.width < 50 && r.top - top < 60 && !(e.innerText || '').trim(); })
        .sort((a, b) => b.getBoundingClientRect().left - a.getBoundingClientRect().left);
      if (!nav.length) return false;
      nav[0].dispatchEvent(new MouseEvent('click', { bubbles: true }));
      return true;
    });
    if (!clicked) { await shot(page, 'FAIL_month_nav'); throw new Error('Month arrow not found inside the calendar'); }
    await sleep(1200);
    shown = await calMonth();
  }
...
async function schedule(page, post, { dryRun, shot }) {
  ...
  // A caption ending in #tag or @handle leaves a suggestion list open; a later
  // click can accept a suggestion and rewrite the caption. Close it while the
  // caption still has focus. (Not Escape: that opens the cancel-upload prompt.)
  ...
  if (!already) {
    ...
    const pos = await page.evaluate(() => {
      const r = Array.from(document.querySelectorAll('input[type="radio"]')).find(x => x.value === 'schedule');
      if (!r) return null;
      // Prefer the radio circle itself: the centre of the "Schedule (i)" label
      // can land on the info icon, which opens a tooltip and selects nothing.
      const own = r.getBoundingClientRect();
      if (own.width > 0) return { x: own.left + own.width / 2, y: own.top + own.height / 2 };
      let n = r;
  ...
  console.log(`   fields: ${fields.date && fields.date.value} ${fields.time && fields.time.value}`);

  // Verify, don't just print: date, time, radio, and the caption's text.
  const pad = n => String(n).padStart(2, '0');
  const wantDate = `${post.dt.getFullYear()}-${pad(post.dt.getMonth() + 1)}-${pad(post.dt.getDate())}`;
  const wantTime = `${pad(post.dt.getHours())}:${pad(post.dt.getMinutes())}`;
  if (!fields.date || !fields.time || fields.date.value !== wantDate || fields.time.value !== wantTime) {
    await shot(page, 'FAIL_datetime');
    throw new Error(`Date/time did not stick: got ${fields.date && fields.date.value} ${fields.time && fields.time.value}, wanted ${wantDate} ${wantTime}`);
  }
  const state = await page.evaluate(() => ({
    scheduleOn: !!Array.from(document.querySelectorAll('input[type="radio"]')).find(x => x.value === 'schedule' && x.checked),
    caption: ((document.querySelector('[contenteditable="true"]') || {}).innerText || ''),
  }));
  const norm = s => s.replace(/\s+/g, ' ').trim();
  const tokens = post.caption.match(/[@#][A-Za-z0-9_.]*[A-Za-z0-9_]/g) || [];
  const lost = tokens.filter(t => !state.caption.includes(t));
  if (!state.scheduleOn) { await shot(page, 'FAIL_schedule_off'); throw new Error('Schedule radio fell back to Now'); }
  if (lost.length || norm(state.caption) !== norm(post.caption)) {
    await shot(page, 'FAIL_caption_changed');
    throw new Error(`Caption changed in the composer (missing: ${lost.join(', ') || 'text differs'})`);
  }
  await shot(page, 'ready');
  if (dryRun) return { dryRun: true };

  // Submit: relabelled "Schedule", in the footer row with "Save draft".
  // Never accept "Post" here - that label means the radio is on Now and a click
  ...
  const ok = await page.evaluate(() => {
    ...
    const submit = btns.find(x => /^schedule$/i.test(x.t) && Math.abs(x.r.top - draft.r.top) < 25);
  ...
  if (!done) throw new Error('Editor still open after 3 min - UNCONFIRMED. Check TikTok Studio > Manage posts before retrying, or it may double-post.');
