...
async function schedule(page, post, { dryRun, shot }) {
  ...
  // A caption ending in "@handle" leaves Business Suite's mention typeahead open.
  // The next click or keypress then accepts its first suggestion, which silently
  // rewrote "@relo_video." as a mention of a DIFFERENT account ("@relovideo").
  // Dismiss it before touching anything else.
  for (let i = 0; i < 3; i++) {
    const open = await page.evaluate(() =>
      Array.from(document.querySelectorAll('[role="listbox"],[role="option"]'))
        .some(e => e.getBoundingClientRect().width > 0));
    if (!open) break;
    console.log('   closing the mention typeahead');
  ...
  // Re-read the caption AFTER all the date/time clicking: a stray keypress can
  // accept a mention suggestion and rewrite a handle. Every @handle in the
  // manifest caption must still be present verbatim.
  const finalCaption = await page.evaluate(() =>
    (document.querySelector('[contenteditable="true"]') || {}).innerText || '');
  const norm = s => s.replace(/\s+/g, ' ').trim();
  const handles = post.caption.match(/@[A-Za-z0-9_.]+[A-Za-z0-9_]/g) || [];
  const lost = handles.filter(h => !finalCaption.includes(h));
  if (lost.length || norm(finalCaption).length < norm(post.caption).length - 5) {
    await shot(page, 'FAIL_caption_changed');
    throw new Error(`Caption changed in the composer (missing: ${lost.join(', ') || 'text'}); ends "${norm(finalCaption).slice(-40)}"`);
  }
  ...
  // Business Suite may instead lay a "You scheduled a post through Meta Business
  // Suite" promo modal over the still-rendered composer. That IS success, and
  // treating it as failure invites a re-run that double-posts.
  const closed = await page.waitForFunction(() => {
    const body = document.body.innerText || '';
    if (/You scheduled a post/i.test(body)) return true;
    const composerOpen = !!document.querySelector('[contenteditable="true"]')
      && /Create post|Post details/i.test(body);
    return !composerOpen;
  }, { timeout: 90000, polling: 2000 }).then(() => true).catch(() => false);

  await shot(page, 'confirmation');
  // Dismiss the promo so the next run starts clean.
  await page.evaluate(() => {
    const d = Array.from(document.querySelectorAll('[role="button"],button'))
      .find(e => /^done$/i.test((e.innerText || '').trim()) && e.getBoundingClientRect().width > 0);
    if (d) d.click();
  }).catch(() => {});
  await sleep(2000);
  if (!closed) {
