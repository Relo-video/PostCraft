...
async function attachMedia(page, host, post, shot) {
  ...
  if (post.type === 'document') {
    ...
    await clickIn(page, host,
      root => Array.from(root.querySelectorAll('button,[role="button"]'))
        .find(b => (b.getAttribute('aria-label') || '').trim() === 'More'),
      12000, 'the More button');
    await sleep(1800);
    await clickIn(page, host,
      root => Array.from(root.querySelectorAll('button,[role="button"]'))
        .find(b => ((b.getAttribute('aria-label') || '') + ' ' + (b.innerText || ''))
          .toLowerCase().includes('add a document')),
    ...
    await clickIn(page, host,
      root => Array.from(root.querySelectorAll('button,[role="button"]'))
        .find(b => (b.getAttribute('aria-label') || '').trim() === 'Add media'),
  ...
  await clickIn(page, host,
    root => Array.from(root.querySelectorAll('button')).find(b => {
      const t = (b.innerText || '').trim();
      return (t === 'Next' || t === 'Done') && !b.disabled && (b.offsetWidth > 0 || b.offsetHeight > 0);
    }),
